...
const USER_EMAIL = __ENV.TEST_USER_EMAIL || '';
...
export function setup() {
  ...
  if (SIGNUP_USER && !USER_EMAIL) {
    ...
    }, { name: 'POST /api/v1/auth/signup setup' }, [200, 409]);
...
function request(method, path, body, params, expectedStatuses) {
  ...
  const ok = expectedStatuses.indexOf(res.status) !== -1;
  unexpectedStatus.add(!ok, { endpoint: requestParams.name || `${method} ${path}` });
  check(res, {
