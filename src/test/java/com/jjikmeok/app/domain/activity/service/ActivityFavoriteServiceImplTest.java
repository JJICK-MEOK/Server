...
class ActivityFavoriteServiceImplTest {

    @Mock ActivityFavoriteRepository favoriteRepository;
    @Mock ActivityRepository activityRepository;
    @Mock UserRepository userRepository;
    ActivityFavoriteServiceImpl service;

    @BeforeEach
    void setUp() {
        service = new ActivityFavoriteServiceImpl(favoriteRepository, activityRepository, userRepository);
    }
    ...
    void getFavorites_returnsUserFavorites() {
        ...
        List<ActivityFavoriteResponse> responses = service.getFavorites(1L);
    ...
    void createFavorite_increasesLikeCount() {
        ...
        when(favoriteRepository.save(any(ActivityFavorite.class))).thenAnswer(invocation -> {
            ActivityFavorite saved = invocation.getArgument(0);
            setId(saved, 3L);
            return saved;
        });

        service.createFavorite(1L, new ActivityFavoriteRequest(2L));
    ...
    void deleteFavorite_decreasesLikeCount() {
        ...
        ActivityFavorite favorite = favorite(3L, user, activity);
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        when(activityRepository.findById(2L)).thenReturn(Optional.of(activity));
    ...
    private Activity activity(Long id) {
        ...
        Activity activity = Activity.builder()
                .region(region).title("활동").uri("https://example.com").location("장소")
    ...
    private ActivityFavorite favorite(Long id, User user, Activity activity) {
        ActivityFavorite favorite = ActivityFavorite.create(user, activity);
        setId(favorite, id);
        return favorite;
    }
