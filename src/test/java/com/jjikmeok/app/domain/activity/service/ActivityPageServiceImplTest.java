...
class ActivityPageServiceImplTest {
    ...
    private ActivityPageServiceImpl activityPageService;

    @BeforeEach
    void setUp() {
        activityPageService = new ActivityPageServiceImpl(
                activityRepository,
    ...
    void getDetailPage_returnsScreenReadyDetail() {
        ...
        when(activityRepository.incrementApprovedViewCount(1L, ApprovalStatus.APPROVED)).thenReturn(1);
        when(activityRepository.findApprovedByIdWithRegion(1L, ApprovalStatus.APPROVED)).thenReturn(Optional.of(activity));
    ...
    void getDetailPage_whenActivityNotFound_throwsActivityNotFound() {
        when(activityRepository.incrementApprovedViewCount(1L, ApprovalStatus.APPROVED)).thenReturn(0);
        ...
        verify(activityRepository).incrementApprovedViewCount(1L, ApprovalStatus.APPROVED);
    }
